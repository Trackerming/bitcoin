...
bool getAddressFromIndex(const int &type, const uint160 &hash, std::string &address) {
  if (type == 2) {
    // CTxDestination dest = ScriptHash(hash);
    address = EncodeDestination(ScriptHash(hash));
  } else if (type == 1) {
    // CTxDestination dest = PKHash(hash);
...
bool getAddressFromParams(const UniValue &params, std::vector<std::pair<uint160, int>> &addresses) {
  if (params[0].isStr()) {
    ...
    if (!addressValues.isArray()) {
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "addresses are expected to be an array");
    }
...
UniValue getaddressutxos(const JSONRPCRequest &request) {
  ...
  for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>::iterator it = unspentOutputs.begin();
...
UniValue getaddressbalance(const JSONRPCRequest &request) {
    ...
    CAmount balance = 0;
    CAmount received = 0;
    LogPrint(BCLog::RPC, "the address index size is %d \n", addressIndex.size());
    for (std::vector<std::pair<CAddressIndexKey, CAmount>>::const_iterator it = addressIndex.begin();
         it != addressIndex.end(); it++) {
        LogPrint(BCLog::RPC, "balance = %d\t", it->second);
        if (it->second > 0) {
            received += it->second;
        }
        balance += it->second;
