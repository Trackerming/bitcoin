...
bool CBlockTreeDB::UpdateSpentIndex(const std::vector<std::pair<CSpentIndexKey, CSpentIndexValue> > &vect) {
  CDBBatch batch(*this);
  for (std::vector<std::pair<CSpentIndexKey, CSpentIndexValue>>::const_iterator it = vect.begin();
       it != vect.end(); it++) {
    if (it->second.IsNull()) {
...
bool CBlockTreeDB::UpdateAddressUnspentIndex(
  const std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &vect) {
  CDBBatch batch(*this);
  for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue>>::const_iterator it = vect.begin();
       it != vect.end(); it++) {
    if (it->second.IsNull()) {
  ...
  while (pcursor->Valid()) {
    ...
    if (pcursor->GetKey(key) && key.first == DB_ADDRESSUNSPENTINDEX && key.second.hashBytes == addressHash) {
      ...
      if (pcursor->GetValue(nValue)) {
        ...
        return error("fail to get address unspent value");
      }
    } else {
      break;
    }
...
bool CBlockTreeDB::ReadAddressIndex(uint160 addressHash, int type,
                                    std::vector<std::pair<CAddressIndexKey, CAmount> > &addressIndex, int start,
                                    int end) {
  std::unique_ptr<CDBIterator> pcursor(NewIterator());
  if (start > 0 && end > 0) {
...
bool CBlockTreeDB::ReadTimestampIndex(const unsigned int &high, const unsigned int &low, const bool fActiveOnly,
  ...
  while (pcursor->Valid()) {
    ...
    if (pcursor->GetKey(key) && key.first == DB_TIMESTAMPINDEX && key.second.timestamp < high) {
      if (fActiveOnly) {
        if (HasOnchainActive(key.second.blockHash)) {
...
bool CBlockTreeDB::ReadTimestampBlockIndex(const uint256 &hash, unsigned int &logicalTS) {
