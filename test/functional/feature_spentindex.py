...
class SpentIndexTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-debug","-deprecatedrpc=signrawtransaction"],["-debug", "-spentindex","-deprecatedrpc=signrawtransaction"],
                           ["-debug", "-spentindex","-deprecatedrpc=signrawtransaction"], ["-debug", "-spentindex", "-txindex","-deprecatedrpc=signrawtransaction"]]
    ...
    def run_test(self):
        print("Mining blocks...")
        coinbase_address_0 = self.nodes[0].getnewaddress()
        self.nodes[0].generatetoaddress(105, coinbase_address_0)
        self.sync_all()
        ...
        self.nodes[0].generatetoaddress(1, coinbase_address_0)
        self.sync_all()

        print("Testing get spentinfo method...")
        ...
        # Check that verbose raw transaction include spent info
        ...
        block_hash = self.nodes[0].generatetoaddress(1, coinbase_address_0)
        self.sync_all()
        ...
        assert_equal(len(block["deltas"][0]["outputs"]), 1) #现有方式需要指定挖矿奖励地址此处应该有一个vout
