...
class AddressIndexTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-debug", "-deprecatedrpc=signrawtransaction"],
                           ["-debug", "-addressindex=1", "-deprecatedrpc=signrawtransaction"],
                           ["-debug", "-addressindex=1", "-deprecatedrpc=signrawtransaction"],
                           ["-debug", "-addressindex=1", "-deprecatedrpc=signrawtransaction"]]
    ...
    def run_test(self):
        ...
        self.nodes[0].generatetoaddress(105, coinbase_address)
        self.sync_all()
        ...
        tx.vout = [CTxOut(100000000, scriptPubKey), CTxOut(4899000000, scriptPubKey)]
        tx.rehash()

        signed_tx = self.nodes[0].signrawtransactionwithwallet(binascii.hexlify(tx.serialize()).decode("utf-8"))
        sent_txid = self.nodes[0].sendrawtransaction(signed_tx["hex"])
        ...
        balance0 = self.nodes[1].getaddressbalance("2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br")
        assert_equal(balance0["balance"], 45 * 100000000 + 4899000000 + 100000000)
        ...
        tx.rehash()
        signed_tx = self.nodes[0].signrawtransactionwithwallet(binascii.hexlify(tx.serialize()).decode("utf-8"))
        spending_txid = self.nodes[0].sendrawtransaction(signed_tx["hex"])
        self.nodes[0].generatetoaddress(1, coinbase_address)
        self.sync_all()
        ...
        coinbase_address2 = self.nodes[2].getnewaddress()
        self.nodes[2].generatetoaddress(150, coinbase_address2)

        txidsort1 = self.nodes[2].sendtoaddress(address2, 50)
        self.nodes[2].generatetoaddress(1, coinbase_address2)
        ...
        addressHash3 = bytes(
            [170, 152, 114, 181, 187, 205, 181, 17, 216, 158, 14, 17, 170, 39, 218, 115, 253, 44, 63, 80])
        ...
        memtxid1 = self.nodes[2].sendrawtransaction(signed_tx["hex"])
        time.sleep(2)
        ...
        memtxid2 = self.nodes[2].sendrawtransaction(signed_tx2["hex"])
        time.sleep(2)
        ...
        blk_hashes = self.nodes[2].generatetoaddress(1, coinbase_address2);
        self.sync_all();
        ...
        memtxid3 = self.nodes[2].sendrawtransaction(signed_tx3["hex"])
        time.sleep(2)
        ...
        address1hash = bytes(
            [193, 146, 191, 247, 81, 175, 142, 254, 193, 81, 53, 212, 43, 254, 237, 249, 26, 111, 62, 52])
        ...
        signed_tx = self.nodes[0].signrawtransactionwithwallet(binascii.hexlify(tx.serialize()).decode("utf-8"))
        mem_txid = self.nodes[0].sendrawtransaction(signed_tx["hex"])
